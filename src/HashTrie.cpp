...
//	cout << "HashTrieNode::compactData - childs_vector.size(): " << childs_vector.size() << "\n";
	for(unsigned int i = 0; i < childs_vector.size(); ++i){
		...
		// If the path to the parent is already greater than MAX_LEN_HASH, omit this node
		// If there is no valid bits inside the range of MAX_LEN_HASH, also omit it
		if( ini_path >= MAX_LEN_HASH || ini_path >= (len_path[cur_pos] & MAX_LEN_HASH) ){
			len_hash[cur_pos] = 0;
		}
		else{
			// MAX LEN HASH
...
//			while( (len_path[cur_pos] & (mask<<1)) > ini_path){
			while( (len_path[cur_pos] & (mask<<1) & MAX_LEN_HASH) > ini_path){
				mask <<= 1;
//				cout << "HashTrieNode::compactData - mask: " << mask << " for " << (len_path[cur_pos] & mask & MAX_LEN_HASH) << " > " << ini_path << "\n";
			}
			len_hash[cur_pos] = mask & len_path[cur_pos] & MAX_LEN_HASH;
//			cout << "HashTrieNode::compactData - len_hash[" << cur_pos << "]: " << len_hash[cur_pos] << " for (" << mask << " & " << len_path[cur_pos] << " & " << MAX_LEN_HASH << ")\n";
			
			if( len_hash[cur_pos] > MAX_LEN_HASH ){
				cerr << "HashTrieNode::compactData - Error in len_hash[" << cur_pos << "]: " << len_hash[cur_pos] << "\n";
				exit(0);
			}
		}
		if( len_hash[cur_pos] != 0 && len_hash[cur_pos] <= ini_path ){
			cerr << "HashTrieNode::compactData - Error in len_hash[" << cur_pos << "]: " << len_hash[cur_pos] << "\n";
			exit(0);
			len_hash[cur_pos] = 0;
		}
...
HashTrie::HashTrie(){
	...
	pi_inv = NULL;
	n_collisions = 0;
	nano_search_global = 0;
	nano_search_collisions = 0;
}
...
void HashTrie::build(const char *full_text, unsigned int _len_text, vector<unsigned int> &factors_start, int_vector<> *_arr_factors, KarpRabin *_karp_rabin, KarpRabinFactorsSuffixes *kr_factors, bool reverse){
	...
	cout << "HashTrie::build - Start (full text of " << len_text << ", " << factors_start.size() << " factors, reverse: " << reverse << ", MAX_LEN_HASH: " << MAX_LEN_HASH << ")\n";
...
//		unsigned int lu = select1_b->operator()(min_factor_pos + 2) - pu;
		...
		if( hash_len > MAX_LEN_HASH ){
			cerr << "HashTrie::prepareHashMap - Error in child_len usado\n";
			exit(0);
		}
...
void HashTrie::printInternal(unsigned int node_pos, unsigned int level){
	...
	cout << "hash: " << hash_childs[node_pos] << ", len: " << len_childs[node_pos] << " , range [" << min_childs[node_pos] << ", " << arr_max_childs[node_pos] << "]\n";
	for(unsigned int i = 0; i < n_childs[node_pos]; ++i){
...
//	cout << "HashTrie::getRangeInternalNoHash - pat: " << pat << " (first_char_pat: " << first_char_pat << ")\n";
	...
	if( pos_child != NOT_FOUND ){
		...
		unsigned int child_len_alt = len_path[pos_child_abs] - len_path[node_pos];
		if( child_len_alt != child_len ){
			cerr << "HashTrie::getRangeInternalNoHash - Error child_len_alt: " << child_len_alt << " vs " << child_len << "\n";
			exit(0);
		}
...
//	cout << "HashTrie::getRangeRevInternalNoHash - pat: " << pat << ", first_char_pat: " << first_char_pat << "\n";
	...
	if( pos_child != NOT_FOUND ){
		...
		unsigned int child_len_alt = len_path[pos_child_abs] - len_path[node_pos];
		if( child_len_alt != child_len ){
			cerr << "HashTrie::getRangeRevInternalNoHash - Error child_len_alt: " << child_len_alt << " vs " << child_len << "\n";
			exit(0);
		}
...
//	NanoTimer timer_global;
//	NanoTimer timer_collisions;
...
//					cout << "HashTrie::getRangeTable - Verificando " << c << " vs " << pattern[pat_ini + i] << "\n";
				...
				nano_search_collisions += timer_collisions.getNanosec();
...
//					cout << "HashTrie::getRangeTable - Verifing node's rest (len_hash: " << len_hash[v_pos] << " -> " << v_len << ")\n";
					unsigned int tail_len = v_len - len_hash[v_pos];
					// El resto podria ser mas largo que el patron
					if( v_len > pat_len ){
						tail_len -= (v_len - pat_len);
						if( tail_len == 0 ){
							break;
						}
					}
					...
					unsigned int tail_hash = karp_rabin->hash(tail_text.c_str(), tail_len);
//					cout << "HashTrie::getRangeTable - tail_hash: " << tail_hash << " (" << tail_text << ")\n";
					unsigned int path_hash = karp_rabin->concat(hash_pat, tail_hash, tail_len);
					
					// String data only to debug
//					string pat_alt_str = pattern.substr(pat_ini, len_hash[v_pos] + tail_len);
//					unsigned int hash_pat_alt_str = karp_rabin->hash(pattern.c_str() + pattern.length() - pat_len, len_hash[v_pos] + tail_len);
//					cout << "HashTrie::getRangeTable - hash_pat_alt_str: " << hash_pat_alt_str << " (" << pat_alt_str << ")\n";

					unsigned int hash_pat_alt = karp_rabin->subtract_prefix(kr_pat_vector[pat_ini + (len_hash[v_pos] + tail_len) - 1], kr_pat_vector[pat_ini - 1], (len_hash[v_pos] + tail_len));
...
//						cout << "HashTrie::getRangeTable - Not Match en nodo completo (" << path_hash << " != " << hash_pat_alt << ")\n";
						v_len = 0;
						break;
					}
					else{
...
//		return getRangeInternal(v_pos, kr_pat_vector, pos, total, karp_rabin, v_max, pattern);
		res = getRangeInternal(v_pos, kr_pat_vector, pos, total, karp_rabin, v_max, pattern);
	}
	
//	nano_search_global += timer_global.getNanosec();
...
//					cout << "HashTrie::getRangeTableRev - Verifing node's rest (len_hash: " << len_hash[v_pos] << " -> " << v_len << ")\n";
					...
					for(unsigned int i = 0; i < tail_len; ++i){
						tail_text += compacted_text->at(tu + lu - len_hash[v_pos] - i - 1);
					}
					unsigned int tail_hash = karp_rabin->hash(tail_text.c_str(), tail_len);
//					cout << "HashTrie::getRangeTableRev - tail_hash: " << tail_hash << " (" << tail_text << ")\n";
					unsigned int path_hash = karp_rabin->concat(hash_pat, tail_hash, tail_len);
					
					// String data only to debug
//					string pat_alt_str = pattern_rev.substr(pat_ini, len_hash[v_pos] + tail_len);
//					unsigned int hash_pat_alt_str = karp_rabin->hash(pattern_rev.c_str() + pattern_rev.length() - pat_len, len_hash[v_pos] + tail_len);
//					cout << "HashTrie::getRangeTableRev - hash_pat_alt_str: " << hash_pat_alt_str << " (" << pat_alt_str << ")\n";

					unsigned int hash_pat_alt = karp_rabin->subtract_prefix(kr_pat_rev_vector[kr_pat_rev_vector.size() - 1 - (pat_len - (len_hash[v_pos] + tail_len))], kr_pat_rev_vector[pat_ini], (len_hash[v_pos] + tail_len));
//					cout << "HashTrie::getRangeTableRev - hash_pat_alt: " << hash_pat_alt << "\n";
					
					if( path_hash != hash_pat_alt ){
//						cout << "HashTrie::getRangeTableRev - Not Match en nodo completo (" << path_hash << " != " << hash_pat_alt << ")\n";
						v_len = 0;
						break;
					}
					else{
